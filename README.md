...
## 📝 Local Embeddings with GPU Acceleration (Custom Fork)

This fork includes local embeddings with GPU acceleration via Vulkan + ROCm for AMD GPUs.

### What Changed

- **Package**: Added `@node-llama-cpp/linux-x64-vulkan` for GPU support
- **Provider**: Local embeddings using `embeddinggemma-300M-Q8_0.gguf` (328MB)
- **GPU Backend**: Vulkan → ROCm on AMD Radeon Graphics (gfx1151)
- **Performance**: ~46% CPU time reduction when GPU is enabled
...
**~/.config/environment.d/openclaw-gateway.conf**:
...
# Monitor GPU during indexing
(openclaw memory index --agent main --force &) && /opt/rocm/bin/rocm-smi --showuse --showpower
```

**Expected Results**:

- GPU usage increases to ~9% during embedding operations
- Power consumption rises from ~14W to ~51W during indexing
- CPU time reduced by ~46% when GPU is enabled

### Hardware

- **GPU**: AMD RYZEN AI MAX+ 395 w/ Radeon 8060S
- **ROCm**: Enabled
- **Vulkan**: `libvulkan_radeon.so`

### Fork Info

- **Fork**: https://github.com/ganpare/openclaw
- **Base**: https://github.com/clawdbot/clawdbot (fork of openclaw/openclaw)
- **Branch**: `main`
